...
func init() {
	var err error
	gui_template, err = template.ParseGlob("static/templates/*.html")
	if err != nil {
...
func playground_space_sep(w http.ResponseWriter, r *http.Request) {
	buf, _ := ioutil.ReadFile("static/examples/space_delim.txt")
...
func playground_csv(w http.ResponseWriter, r *http.Request) {
	...
	buf, _ := ioutil.ReadFile("static/examples/csv.csv")
	input_text := string(buf)
